...
def parse_input_file_for_urls(file_path="ip.txt"):
    if not os.path.exists(file_path):
        print(f"{file_path}文件不存在，无法生成根路径URL")
        return []
    ...
    print(f"已从{file_path}生成 {len(parsed_data)} 个根路径候选URL")
    return parsed_data
...
def main():
    ...
    if not parsed_data:
        print("未从扫描结果中发现可用的Web候选URL，尝试从ip.txt生成根路径候选URL")
        parsed_data = parse_input_file_for_urls()
    if not parsed_data:
